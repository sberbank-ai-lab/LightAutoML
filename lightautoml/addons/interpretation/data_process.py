from typing import List, Iterable, Dict, Tuple, Callable, Optional, Any, Union
...
class BySequenceLengthSampler(Sampler):
    ...
    def __init__(self, data_source: LengthDataset,
...
def get_tokenized(data: 'pd.DataFrame',
                  tokenized_col: str,
                  tokenizer: Callable
                 ) -> List[List[str]]:
    """
...
def get_vocab(tokenized: List[List[str]],
              max_len_vocab: int
...
def get_len_dataset(tokenized: List[List[str]],
                    target: np.ndarray
                   ) -> LengthDataset:
    """
    Get length dataset.
...
def get_len_dataloader(dataset: LengthDataset,
                       batch_size: int = 1,
