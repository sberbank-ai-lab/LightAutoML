...
from lightautoml.addons.uplift.meta_learners import MetaLearner
from lightautoml.addons.uplift.meta_learners import RLearner
from lightautoml.addons.uplift.meta_learners import SLearner
from lightautoml.addons.uplift.meta_learners import TDLearner
...
class AutoUplift(BaseAutoUplift):
    ...
    def __init__(
        ...
        if len(uplift_candidates) > 0:
            if timeout is not None:
                logger.warning(
                    "'timeout' isn't used when 'uplift_candidates' is specified."
                )
    ...
    def _generate_uplift_candidates(self, data: DataFrame, roles):
        ...
        if self.timeout_single_learner is not None:
            ...
            self._tabular_timeout = (
                self.timeout
                if self.timeout is None
                else int(self.timeout / num_tabular_automls)
            )
...
class AutoUpliftTX(BaseAutoUplift):
    ...
    def _prepare_data_for_stage(
        ...
        if len(stage_info) == 1:
            if stage_name == "propensity":
                ...
                train_roles = deepcopy(roles)
                train_roles.pop(treatment_role)
                train_data = train[train[treatment_col] == 0].drop(
                    treatment_col, axis=1
                )
            elif stage_name == "outcome_treatment":
                train_roles = deepcopy(roles)
                train_roles.pop(treatment_role)
                train_data = train[train[treatment_col] == 1].drop(
                    treatment_col, axis=1
                )
            else:
    ...
    def _bl_for_ml(
        ...
        for ml_name, ml_stages in self.__MAP_META_TO_STAGES__.items():
            ready_metalearners.append(
                all(
                    s.full_name() in self._trained_stage_baselearners for s in ml_stages
                )
            )
    ...
    def _get_trained_bl(
        self, metalearner_stage: MLStageFullName, baselearner_name: str
    ):
