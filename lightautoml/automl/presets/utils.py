...
def plot_pdp_with_distribution(test_data, grid, ys, counts, reader, feature_name, individual, top_n_classes, datetime_level):
    ...
    if n_classes == 1:
        data = pd.concat([pd.DataFrame({"x": grid[i], "y": ys[i].ravel(), "unit": np.arange(ys[i].shape[0])}) for i, _ in enumerate(grid)]).reset_index(drop=True)
        if feature_role in ["Numeric", "Datetime"]:
            if individual:
                g0 = sns.lineplot(data=data[data["unit"]>0], x="x", y="y", ax=axs[0], units="unit", estimator=None, alpha=0.05, color="b")
                g0 = sns.lineplot(data=data[data["unit"]==0], x="x", y="y", ax=axs[0], alpha=0.05, color="b", label="individual PDP")
                g0 = sns.lineplot(x=grid, y=data.groupby("x").mean()["y"].values, ax=axs[0], linewidth=2, color="r", label="mean PDP")
            else:
