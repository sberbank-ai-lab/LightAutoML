...
class SeqNumpyPandasDataset(PandasDataset):
    ...
    def __init__(self, data: Optional[DenseSparseArray], features: NpFeatures = (), roles: NpRoles = None, idx: List = (),
                 task: Optional[Task] = None, name: Optional[str] = 'seq', scheme: Optional[dict] = None, **kwargs: np.ndarray):
    ...
    def __getitem__(self, k: Tuple[RowSlice, ColSlice]) -> Union['LAMLDataset', LAMLColumn]:
        ...
        if is_slice:
            ...
            warnings.warn("Resulted sequential dataset may have different structure. It's not recommended to slice new dataset")
