...
from lightautoml.transformers.categorical import TargetEncoder, MultiClassTargetEncoder, LabelEncoder, FreqEncoder, \
    OrdinalEncoder
...
def gini_normalized(y_true: np.ndarray, y_pred: np.ndarray, empty_slice: Optional[np.ndarray] = None):
    ...
    assert y_pred.shape[1] == 1 or y_true.shape[1] == y_pred.shape[1], 'Shape missmatch. Only calculate NxM vs NxM or Nx1 vs NxM'
...
def get_numeric_roles_stat(train: NumpyOrPandas, subsample: Optional[Union[float, int]] = 100000, random_state: int = 42,
    ...
    res = DataFrame(columns=['flg_manual', 'unique', 'unique_rate', 'top_freq_values', 'raw_scores', 'binned_scores',
                             'encoded_scores', 'freq_scores', 'nan_rate'], index=roles_to_identify)
...
def rule_based_roles_guess(stat: DataFrame) -> Dict[str, ColumnRole]:
    ...
    """
    numbers = stat[stat[[x for x in stat.columns if 'rule_' in x]].any(axis=1)]
    ...
    categories['ord_rule'] = categories['unique_rate'] > .01
    ...
    roles_dict = {**roles_dict, **{x: CategoryRole(np.float32,
                                                   encoding_type='int', ordinal=y) for (x, y) in zip(feats, ordinal)}}
    ...
    roles_dict = {**roles_dict, **{x: CategoryRole(np.float32,
                                                   encoding_type='freq', ordinal=y) for (x, y) in zip(feats, ordinal)}}
    ...
    roles_dict = {**roles_dict, **{x: CategoryRole(np.float32,
                                                   encoding_type='auto', ordinal=y) for (x, y) in zip(feats, ordinal)}}
...
def get_category_roles_stat(train: NumpyOrPandas, subsample: Optional[Union[float, int]] = 100000, random_state: int = 42,
    ...
    res = DataFrame(columns=['unique', 'top_freq_values', 'dtype', 'encoded_scores',
                             'freq_scores', 'ord_scores'], index=roles_to_identify)
...
def rule_based_cat_handler_guess(stat: DataFrame) -> Dict[str, ColumnRole]:
    ...
    for enc_type, st in zip(['freq', 'auto', 'ord'], [freqs, auto, ordinals]):
        ...
        roles_dict = {**roles_dict, **{x: CategoryRole(dtype=d, encoding_type=enc_type, ordinal=ordinal)
