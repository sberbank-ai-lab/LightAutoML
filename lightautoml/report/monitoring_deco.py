...
class ThresholdDetector(BaseDetector):
    ...
    def __init__(self, threshold: Union[int, float, np.ndarray] = 0.2, higher_is_better: bool = True,
                 comp: str = 'absolute', rule: str = 'all', store_values: bool = False, base: Optional = None,
                 **kwargs: Any):
...
class DataShiftDetector:
    ...
    def fit(self, train_data, roles):
        ...
        for field_name in self.roles:
            ...
            self.data_info[field_name]['null_detector'] = ThresholdDetector(base=null_values, \
                threshold=self.null_threshold, higher_is_better=False, store_values=True)
            # drop null
            field_series = field_series.dropna()
            if self.roles[field_name].name == 'Numeric':
                self.data_info[field_name]['type'] = 'Numeric'
                self.data_info[field_name]['breakpoints'], self.data_info[field_name]['percents'] = \
                    self._breakpoints_percents(field_series.values)
                self.data_info[field_name]['shift_detector'] = ThresholdDetector(base=0.0, \
                    threshold=self.shift_threshold, higher_is_better=False, store_values=True)
    ...
    def update(self, epoch_data):
        ...
        for field_name in self.data_info:
            ...
            # Numeric variable
            if self.roles[field_name].name == 'Numeric':
                actual_percents = np.histogram(field_series, \
                                               self.data_info[field_name]['breakpoints'])[0] / len(field_series)
            ...
            if self.roles[field_name].name == 'Category':
                ...
                for value, num in Counter(field_series).most_common():
                    ...
                    if value not in self.data_info[field_name]['categories']:
                        new_category_item = {'n_epoch': self.n_epoch, 'value': value, \
...
class ModelShiftDetector(DataShiftDetector):
    ...
    def fit(self, oof_preds):
        ...
        if self.task in ['binary', 'reg']:
            self.model_info['oof_breakpoints'], self.model_info['oof_percents'] = \
                    self._breakpoints_percents(oof_preds)
        else:
            self.model_info['oof_mapping'], self.model_info['oof_percents'], _ = self._category_percents(oof_preds)
        self.model_info['psi_shift_detector'] = ThresholdDetector(base=self.psi_shift_threshold, \
                    threshold=self.shift_threshold, higher_is_better=False, store_values=True)
        self.model_info['mean_shift_detector'] = ThresholdDetector(base=np.mean(oof_preds), \
                    threshold=self.shift_threshold, higher_is_better=False, store_values=True)
    
    def update(self, epoch_preds):
        # Получает вектор предсказаний на эпохе, считает статистику и сравнивает с сохранённой
        if self.task in ['binary', 'reg']:
            actual_percents = np.histogram(epoch_preds, \
                                           self.model_info['oof_breakpoints'])[0] / len(epoch_preds)
...
def plot_null_shift_detector(null_values, thres, path):
    ...
    plt.plot([0, len(null_values)-1], [thres, thres], color='red', lw=2, linestyle='--', label='NULL percentage train');
...
class MonitoringDeco(ReportDeco):
    ...
    def fit_predict(self, *args, **kwargs):
        ...
        self.DSD = DataShiftDetector(null_threshold=self.null_threshold, \
                                     shift_threshold=self.feature_shift_threshold)
        self.MSD = ModelShiftDetector(task=self.task, \
                                      psi_shift_threshold=self.model_shift_threshold)
        ...
        for feature_name in self.DSD.alerts:
            ...
            if any(name in feature_alert for name in ['psi_values', 'null_values']) :
                ...
                if 'psi_values' in feature_alert:
                    ...
                    plot_psi_shift_detector(psi_values=feature_alert['psi_values'], \
                ...
                if 'null_values' in feature_alert:
                    ...
                    plot_null_shift_detector(null_values=feature_alert['null_values'], \
    ...
    def _generate_model_shift_info(self):
        ...
        plot_psi_prediction_shift(psi_values=self.MSD.model_info['psi_shift_detector'].get_values(), \
        ...
        plot_mean_prediction_shift(mean_values=self.MSD.model_info['mean_shift_detector'].get_values(), \
    ...
    def _generate_data_shift_section(self):
        ...
        data_shift_section = env.get_template(self._data_shift_path).render(
            data_shift_info=data_shift_info
        )
